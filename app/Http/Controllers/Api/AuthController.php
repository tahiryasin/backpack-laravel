...
use Laravel\Socialite\One\AbstractProvider as OneProvider;
use Laravel\Socialite\Two\AbstractProvider as TwoProvider;
...
class AuthController extends Controller
{
    ...
    public function social(Request $request, $provider)
    {
        if ($request->has('redirect_uri')) {
            Config::set("services.{$provider}.redirect", $request->get("redirect_uri"));
        } else if (!Config::get("services.{$provider}.redirect")) {
            Config::set("services.{$provider}.redirect", url('/'));
        }


        /** @var OneProvider|TwoProvider $socialite */
        $socialite = Socialite::driver($provider);

        /** @var \Laravel\Socialite\Contracts\User $user */
        if ($socialite instanceof OneProvider) {
            $socialiteUser = $socialite->userFromTokenAndSecret($request->input('access_token'), $request->input('secret'));
        } else {
            $socialite->stateless();
            $socialiteUser = $socialite->userFromToken($request->input('access_token'));
        }

        if (!$socialiteUser) {
            return new JsonResponse([
                'error' => 'unable_to_retrieve',
            ], JsonResponse::HTTP_BAD_REQUEST);
        }
